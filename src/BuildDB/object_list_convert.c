...
void calc_offset()
{
  ...
  for (i = 0 ; i < MAX_OBJECTS ; i++) {
    offset[i] = tmp;
    tmp += strlen(object_list[i].name) + 1; // add 1 for null termination
  }
}
void print_names()
{
  int i, len = 0;
  printf("Char object_names[][] = {\n");
  for (i = 0 ; i < MAX_OBJECTS ; i++) {
    len += strlen(object_list[i].name) + 4; // add 1 for null termination
    if (len > 75) {
      len = strlen(object_list[i].name) + 4;
      printf("\n  ");
    }
    printf("\"%s\", ", object_list[i].name);
  }
  printf("\n};\n");
}
void print_structs()
{
  int i;
  char c;
  printf("treasure_type object_list[MAX_OBJECTS] = {\n");
  printf("  // name, flags, type, c, p1,  $, cat, #, wt, +hit,+dam, ac,+ac, dam2, lev\n");
  //  for (i = 0 ; i < 60 ; i++) {
  for (i = 0 ; i < MAX_OBJECTS ; i++) {
    printf("{%4d,",     offset[i]);
    printf("0x%08xL,",  object_list[i].flags);
    printf("%3d,",      object_list[i].tval);
    c = object_list[i].tchar;
    switch(c) {
    case '\\': case '\'':
      printf("'\\%c',", c);
      break;
    ...
    printf("%4d,",      object_list[i].p1);
    printf("%4ld,",     object_list[i].cost);
    printf("%3d,",      object_list[i].subval);
    printf("%d,",       object_list[i].number);
    printf("%4d,",       object_list[i].weight);
    printf("%2d,",       object_list[i].tohit);
    printf("%2d,",       object_list[i].todam);
    printf("%2d,",       object_list[i].ac);
    printf("%2d,",       object_list[i].toac);
    printf("{%2d,%2d},",   object_list[i].damage[0],object_list[i].damage[1]);
    printf("%3d },",       object_list[i].level);
    printf("\n");
  }
  printf("};\n");
}
...
{
  Short i;
  printf("Short t_level[MAX_OBJ_LEVEL+1] = {");
  for (i = 0 ; i < MAX_OBJ_LEVEL+1 ; i++) {
    if (i % 12 == 0)
      printf("\n  ");
    printf("%4d,", t_level[i]);
  }
...
void print_sorted_objects()
{
  Short i;
  printf("Short sorted_objects[MAX_DUNGEON_OBJ] = {");
  for (i = 0 ; i < MAX_DUNGEON_OBJ ; i++) {
    if (i % 12 == 0)
      printf("\n  ");
    printf("%4d,", sorted_objects[i]);
  }
  printf("\n};\n");
}
...
void main()
{
  // used for fueling object_list.c:
  // calc_offset();
  //  print_names();
  //  print_structs();

  // used for fueling tlevel.c and sorted_objects.c:
  init_t_level();
  print_t_level();
  print_sorted_objects();
...
void foo()
{
  ...
  for (i = 0 ; i < MAX_OBJECTS ; i++) {
    avg_len += strlen(object_list[i].name) + 1;
  }
  printf("total string %d\n", avg_len);
  tmp = 32 * MAX_OBJECTS - avg_len;
  avg_len /= MAX_OBJECTS;
  printf("average string %d\n", avg_len);
  printf("savings of %d\n", tmp);
